...
defmodule ClosestPoints do
  ...
  def subtractPoint(pA, pB) do
    pA |> IO.inspect()
    |> Enum.zip(pB) |> IO.inspect()
    |> Enum.map(fn {a, b} -> a - b end) |> IO.inspect()
  end
...
defmodule Script do
  ...
  def usage() do
    ...
    IO.puts(
      "  closest_points [--origin ORIGIN] [--points POINTS] [--result-count RESULT_COUNT] [--verbose] [--help]"
    )

    IO.puts("    -o | --origin ORIGIN             : coordinates of origin, default \"0,0\"")

    IO.puts(
      "    -p | --points POINTS             : list of points to use, default \"[0,1],[-1,0]\""
    )

    IO.puts(
      "    -r | --result-count RESULT_COUNT : number of closest points to return, default 1"
    )
    ...
    IO.puts(
      "  closest_points -p [0,0,0,0],[-1,3,3,3],[3,-1,3,3],[3,3,-1,3],[3,3,3,-1] -o 0,1,2,3 -r 2"
    )
  end
