...
defmodule Script do
  ...
  def conversion_list(conversions, from, to) do
    {result, _} =
      conversions
      |> conversion_flatmap([], from) |> IO.inspect()
      ...
      |> Enum.reduce({[],nil},
        fn
          ...
          ^to, {l,pl} ->
            IO.inspect("three")
            {l++[pl++[to]], nil} |> IO.inspect()
          next, {l,pl} ->
            IO.inspect("four")
            {l,pl++[next]} |> IO.inspect()
          _, a ->
            IO.inspect("five")
            a |> IO.inspect()
        end
  ...
  def convert(rates, from, to) do
    ...
    if 0 < length(result) do
      ...
      IO.inspect("NULL")
      build_rate_chain_list(rates, to, from) |> IO.inspect()
      |> Enum.map(fn r -> {r |> Enum.reverse, 1/resolve_rate_chain(rates, r, 1)} end) |> IO.inspect()
    end
  end
