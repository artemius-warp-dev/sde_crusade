defmodule RationalNumbers do
  ...
  def reduce({a, b}) do
    gcd = gcd(Kernel.abs(a), Kernel.abs(b))

    cond do
      rem(a, gcd) != 0 or rem(b, gcd) != 0 -> {a, b}
      true -> {div(a, gcd), div(b, gcd)}
    end
    |> to_standart_form()
  end

  defp to_standart_form({a, b}) when b < 0, do: {a * -1, b * -1}
  defp to_standart_form(r), do: r

  defp gcd(a, b) when rem(a, b) == 0, do: b
  defp gcd(a, b) when b > a, do: gcd(b, a)
  defp gcd(a, b) when rem(a, b) > 0, do: gcd(a, rem(a, b))
end
